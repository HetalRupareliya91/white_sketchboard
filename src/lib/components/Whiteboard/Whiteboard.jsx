...
const Whiteboard = ({
  ...
  function handleRedoAll() {
    while (undoManager.hasRedo()) {
      undoManager.redo();
    }
  }
  function handleSaveCanvasState() {
    const newCanvasState = board.canvas.toJSON();
    setCanvasStates((prevStates) => [...prevStates, newCanvasState]);
    const newimgpath = board.canvas.toDataURL('image/png');
    // const w = window.open('about:blank', 'image from canvas');
    setImageList((imgpath) => [...imgpath, newimgpath]);
    //setImageList(d);
    // console.log('Saved!');
  ...
  return (
    <WhiteBoardS ref={whiteboardRef}>
      <ToolbarHolderS>
        ...
        <ZoomBarS>
          <ToolbarItemS>
            <ButtonS type="button" onClick={handleUndoAll} >
              Clear All
            </ButtonS>
          </ToolbarItemS>
